...
class HotspotMonitor:
    ...
    def load_protected_areas(self):
        try:
            if os.path.exists('KK_NTB.geojson'):
                gdf = gpd.read_file('KK_NTB.geojson')
                logging.info(f"Loaded {len(gdf)} features from KK_NTB.geojson")
        ...
        raise FileNotFoundError("KK_NTB.geojson not found. Cannot initialize HotspotMonitor.")
